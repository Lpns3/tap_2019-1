...
module.exports = {
    ...
    async index (req, res) {
        ...
        const recibos = await Recibo.paginate({}, { page, limit: 10 });
    ...
    async details (req, res) {
        ...
        const recibos = await Recibo.findById(req.params.id);
    ...
    async insert (req, res) {
        const recibos = await Recibo.create(req.body);
    ...
    async update (req, res) {
        const recibos = await Recibo.findByIdAndUpdate(req.params.id, req.body, { new: true });
    ...
    async delete (req, res) {
        await Recibo.findByIdAndRemove(req.params.id);
